...
{
    stream << req.method << " " << req.uri << " HTTP/" << req.versionMajor << "." << req.versionMinor << "\n";

    for (std::vector<httpparser::Request::HeaderItem>::const_iterator it = req.headers.begin(); it != req.headers.end();
         ++it)
    {
...
{
    stream << "HTTP/" << resp.versionMajor << "." << resp.versionMinor << " " << resp.statusCode << " " << resp.status
           << "\n";
...
public:
    ...
    template <size_t N> RequestDsl& content(const char (&buf)[N])
    {
        request.content = std::vector<char>(buf, buf + N - 1);
        return *this;
    }

    operator httpparser::Request() const { return request; }
...
public:
    ...
    template <size_t N> ResponseDsl& body(const char (&buf)[N])
    {
        response.content = std::vector<char>(buf, buf + N - 1);
        return *this;
    }

    operator httpparser::Response() const { return response; }
