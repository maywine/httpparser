...
BOOST_FIXTURE_TEST_CASE(get_http_09, RequestFixture)
{
    ...
    Request should   = RequestDsl().method("GET").uri("/uri").version(0, 9);
...
BOOST_FIXTURE_TEST_CASE(get_http_09_uri_with_query_string, RequestFixture)
{
    ...
    Request result   = parse(text);
    Request should   = RequestDsl().method("GET").uri("/uri?arg1=test;arg1=%20%21;arg3=test").version(0, 9);
...
BOOST_FIXTURE_TEST_CASE(post_http_11, RequestFixture)
{
    ...
    Request result   = parse(text);
    Request should   = RequestDsl().method("POST").uri("/uri").version(1, 1).keepAlive(true);
...
BOOST_FIXTURE_TEST_CASE(post_http_11_with_header_field, RequestFixture)
{
    ...
    Request result = parse(text);
    Request should =
        RequestDsl().method("POST").uri("/uri").version(1, 1).header("X-Custom-Header", "header value").keepAlive(true);
